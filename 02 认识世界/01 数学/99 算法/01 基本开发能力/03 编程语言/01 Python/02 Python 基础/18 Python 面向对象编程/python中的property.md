...
之前就想，Python中的属性到底是怎么做成类似 private 的？还是直接暴露的？还是通过 c#类似的 getset？还是只是通过名字前面加上'__'来实现的？。。现在才知道是使用的 property
...
注：**这里对描述器还是有些不清楚，而且，为什么生成的这个 MyProperty 的对象叫做 score？没有说吧？而且为什么 s.score=95 就会直接调用到 score 的 set 函数？**
