...
PyObject* PyUnicode_FromFormat(const char *format, ...)
   ...
   |                     |                       | "printf("%lli")".                |
   +---------------------+-----------------------+----------------------------------+
   | "%llu"              | 无符号 long long      | Exactly equivalent to            |
   |                     |                       | "printf("%llu")".                |
   +---------------------+-----------------------+----------------------------------+
   ...
   |                     |                       | "printf("%zu")".                 |
   +---------------------+-----------------------+----------------------------------+
   | "%i"                | 整型                  | Exactly equivalent to            |
   |                     |                       | "printf("%i")".                  |
   +---------------------+-----------------------+----------------------------------+
   | "%x"                | 整型                  | Exactly equivalent to            |
   |                     |                       | "printf("%x")".                  |
   +---------------------+-----------------------+----------------------------------+
   ...
   注解: The width formatter unit is number of characters rather
     than bytes. The precision formatter unit is number of bytes for
     ""%s"" and ""%V"" (if the "PyObject*" argument is NULL), and a
     number of characters for ""%A"", ""%U"", ""%S"", ""%R"" and
...
PyObject* PyUnicode_DecodeLocaleAndSize(const char *str, Py_ssize_t len, const char *errors)
   ...
   383**). The decoder uses ""strict"" error handler if *errors* is
   ...
   在 3.6.5 版更改: The function now also uses the current locale
   encoding for the "surrogateescape" error handler. Previously,
   "Py_DecodeLocale()" was used for the "surrogateescape", and the
...
PyObject* PyUnicode_EncodeLocale(PyObject *unicode, const char *errors)
   ...
   *errors* is "NULL". Return a "bytes" object. *unicode* cannot
   ...
   在 3.6.5 版更改: The function now also uses the current locale
   encoding for the "surrogateescape" error handler. Previously,
   "Py_EncodeLocale()" was used for the "surrogateescape", and the
   current locale encoding was used for "strict".
...
wchar_t* PyUnicode_AsWideCharString(PyObject *unicode, Py_ssize_t *size)
   ...
   to free it) on success. On error, returns *NULL*, **size* is
   undefined and raises a "MemoryError". Note that the resulting
